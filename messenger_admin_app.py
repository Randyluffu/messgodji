...
открывает главное окно. Повторный запуск exe, пока приложение уже
работает, не плодит вторую копию — просто поднимает окно уже
запущенного экземпляра (как и положено обычному трей-приложению).
...
# ───────────────────────── DPI / скруглённые окна / единственный экземпляр ─────────────────────────
...
def setup_ttk_style():
    ...
    style.configure('Godji.Vertical.TScrollbar', background=PANEL_BORDER, troughcolor=BG,
                     bordercolor=BG, arrowcolor=MUTED, relief='flat', gripcount=0, width=10)
...
def signal_existing_instance():
    try:
        ...
        print('[messenger] Уже запущено — поднимаю существующее окно')
...
chat_active = {}
pc_nicknames = {}
pc_muted_until = {}   # {pc: ts} — временное отключение звука по конкретному ПК
...
def get_local_ipv4_list():
    """Список IPv4-адресов всех сетевых адаптеров этого ПК (для рассылки
    маячка на каждый, а не только на маршрут по умолчанию)."""
    ips = set()
    try:
...
class ToastStack:
    ...
    def show(self, pc, text, on_click=None):
        ...
        if _notify_sound[0] and not is_pc_muted(pc):
            try:
...
class AdminChatWindow:
    def __init__(self, root, pc):
        ...
        outer.bind('<Button-3>', self._mute_menu)
        ...
        send_btn = rounded_button(inner, '➤', lambda: self.send_text(), w=40, h=34)
    ...
    def _mute_menu(self, event):
        menu = tk.Menu(self.win, tearoff=0, bg=PANEL, fg=TEXT_LIGHT, activebackground=ACCENT,
                        activeforeground='#fff', bd=0, relief='flat')
        menu.add_command(label='Звук — без изменений', state='disabled')
        menu.add_separator()
        for mins in (5, 10, 15, 30):
            menu.add_command(label='Заглушить на %d мин' % mins,
                               command=lambda m=mins: (mute_pc_for(self.pc, minutes=m), self._refresh_mute_dot()))
        menu.add_command(label='Заглушить на весь сеанс',
                           command=lambda: (mute_pc_for(self.pc, session=True), self._refresh_mute_dot()))
        menu.add_separator()
        menu.add_command(label='Включить звук обратно',
                           command=lambda: (unmute_pc(self.pc), self._refresh_mute_dot()))
        menu.tk_popup(event.x_root, event.y_root)
...
class MainWindow:
    ...
    def _scroll_list(self, parent):
        listwrap = tk.Frame(parent, bg=BG)
        listwrap.pack(fill='both', expand=True, padx=12, pady=4)
        canvas = tk.Canvas(listwrap, bg=BG, highlightthickness=0)
        vsb = ttk.Scrollbar(listwrap, orient='vertical', command=canvas.yview, style='Godji.Vertical.TScrollbar')
    ...
    def show_recent(self):
        ...
        inner = self._scroll_list(self.content)
    ...
    def show_settings(self):
        ...
        tk.Label(wrap, text='Временно отключить звук по конкретному ПК — правая кнопка мыши\n'
                            'внутри открытого чата с этим ПК.', bg=BG, fg=MUTED, font=('Segoe UI', 8),
                  justify='left').pack(anchor='w', padx=14, pady=(16, 0))
...
def start_services():
    ...
    def poll_show_event():
        if _show_window_event.is_set():
            ...
            main_win.show()
        root.after(300, poll_show_event)
    root.after(300, poll_show_event)
