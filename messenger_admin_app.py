...
# ───────────────────────── скруглённые окна ─────────────────────────
def _is_windows11_or_later():
    try:
        v = sys.getwindowsversion()
        return v.major > 10 or (v.major == 10 and v.build >= 22000)
    except Exception:
        return False
...
def force_dark_titlebar(win):
    """Красит нативную (нефреймлесс) шапку окна в тёмный цвет — Windows 10 1809+/11."""
    try:
        win.update_idletasks()
        hwnd = win.winfo_id()
...
pc_nicknames = {}
pc_muted_until = {}
pc_cleared_at = {}   # {pc: ts} — когда админ последний раз чистил чат, чтобы клиент/ERP тоже стёрли локально
...
def clear_messages(pc):
    with lock:
        ...
        pc_cleared_at[pc] = now()


def get_cleared_at(pc):
    with lock:
        return pc_cleared_at.get(pc, 0)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path == '/clear_state':
            pc = (qs.get('pc') or [''])[0]
            return self._json(200, {'clearedAt': get_cleared_at(pc)})
...
def _new_icon(size=28):
    return Image.new('RGBA', (size, size), (0, 0, 0, 0))


def build_emoji_icons():
    """Рисует набор реакций вручную через Pillow — гарантированно цветные,
    не зависят от того, поддерживает ли Windows/шрифт цветные эмодзи-глифы."""
    if _EMOJI_ICON_CACHE:
        return _EMOJI_ICON_CACHE
    s = 30

    def face(bg, eyes='dots', mouth='smile'):
        img = _new_icon(s)
        d = ImageDraw.Draw(img)
        d.ellipse((1, 1, s - 2, s - 2), fill=bg)
        if eyes == 'dots':
            d.ellipse((9, 11, 12, 14), fill='#1a1a1a')
            d.ellipse((18, 11, 21, 14), fill='#1a1a1a')
        elif eyes == 'angry':
            d.line((7, 10, 13, 13), fill='#1a1a1a', width=2)
            d.line((23, 10, 17, 13), fill='#1a1a1a', width=2)
        elif eyes == 'closed':
            d.line((8, 12, 13, 12), fill='#1a1a1a', width=2)
            d.line((17, 12, 22, 12), fill='#1a1a1a', width=2)
        if mouth == 'smile':
            d.arc((8, 12, 22, 24), start=15, end=165, fill='#1a1a1a', width=2)
        elif mouth == 'bigsmile':
            d.pieslice((8, 13, 22, 25), start=15, end=165, fill='#7a2b12')
        elif mouth == 'frown':
            d.arc((8, 17, 22, 27), start=200, end=340, fill='#1a1a1a', width=2)
        elif mouth == 'flat':
            d.line((11, 19, 19, 19), fill='#1a1a1a', width=2)
        return img

    icons = {}
    icons['😀'] = face('#ffcc33', 'dots', 'smile')
    icons['😂'] = face('#ffcc33', 'closed', 'bigsmile')
    icons['😢'] = face('#ffcc33', 'dots', 'frown')
    icons['😡'] = face('#ff8a3d', 'angry', 'flat')

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.polygon([(15, 26), (3, 12), (3, 8), (7, 4), (11, 4), (15, 9), (19, 4), (23, 4), (27, 8), (27, 12)],
              fill='#e0293c')
    icons['❤️'] = img

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.polygon([(16, 2), (11, 15), (14, 15), (10, 27), (21, 12), (17, 12), (21, 2)], fill='#ff7a1a')
    icons['🔥'] = img

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.rounded_rectangle((1, 1, s - 2, s - 2), radius=8, fill='#3ecf5e')
    d.line((8, 15, 13, 21), fill='#fff', width=3)
    d.line((13, 21, 23, 8), fill='#fff', width=3)
    icons['✅'] = img

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.rounded_rectangle((1, 1, s - 2, s - 2), radius=8, fill='#e0393f')
    d.line((9, 9, 21, 21), fill='#fff', width=3)
    d.line((21, 9, 9, 21), fill='#fff', width=3)
    icons['❌'] = img

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.ellipse((2, 2, s - 3, s - 3), outline='#3a2226', width=2, fill='#fff')
    d.line((15, 15, 15, 7), fill='#1a1a1a', width=2)
    d.line((15, 15, 21, 17), fill='#1a1a1a', width=2)
    icons['⏰'] = img

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.ellipse((3, 6, s - 4, s - 3), fill='#3ecf5e')
    d.text((11, 11), '$', fill='#fff')
    icons['💰'] = img

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.rounded_rectangle((2, 9, s - 3, s - 10), radius=8, fill='#8a6fd6')
    d.ellipse((6, 13, 10, 17), fill='#fff')
    d.ellipse((20, 12, 24, 16), fill='#fff')
    icons['🎮'] = img

    img = _new_icon(s); d = ImageDraw.Draw(img)
    d.ellipse((3, 3, s - 4, s - 4), outline='#e0293c', width=2)
    d.text((11, 8), '?', fill='#e0293c')
    icons['❓'] = img

    for ch, im in icons.items():
        _EMOJI_ICON_CACHE[ch] = ImageTk.PhotoImage(im)
    return _EMOJI_ICON_CACHE
...
class AdminChatWindow:
    ...
    def toggle_emoji_panel(self):
        ...
        icons = build_emoji_icons()
        cols = 7
        for i, (ch, photo) in enumerate(icons.items()):
            _image_cache.append(photo)
            b = tk.Label(grid, image=photo, bg=PANEL, cursor='hand2', padx=4, pady=4)
            b.grid(row=i // cols, column=i % cols)
            b.bind('<Button-1>', lambda e, ch=ch: self._insert_emoji(ch))
...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        self.expanded = False
        self.win = root
        self.win.title('Godji Messenger — Администратор')
        self.win.configure(bg=BG)
        w, h = 820, 580
        sw, sh = self.win.winfo_screenwidth(), self.win.winfo_screenheight()
        self.win.geometry('%dx%d+%d+%d' % (w, h, (sw - w) // 2, (sh - h) // 2))
        self.win.minsize(600, 420)
        # Обычное окно с системной рамкой — видно в панели задач, по Alt-Tab,
        # не пропадает при клике мимо (это давало overrideredirect раньше).
        ...
        self.show_recent()
        self.win.after(150, force_dark_titlebar, self.win)
    ...
    def _add_nav(self, icon, label, cmd, parent=None, pack_side='top'):
        ...
        icon_wrap = tk.Frame(row, bg=PANEL2, width=self.SIDEBAR_COLLAPSED, height=34)
        icon_wrap.pack(side='left')
        icon_wrap.pack_propagate(False)
        ic = tk.Label(icon_wrap, text=icon, bg=PANEL2, fg=TEXT_LIGHT, font=('Segoe UI', 13))
        ...
        def on_enter(e):
            row.config(bg='#2c1a1d'); icon_wrap.config(bg='#2c1a1d'); ic.config(bg='#2c1a1d'); lbl.config(bg='#2c1a1d')
        def on_leave(e):
            row.config(bg=PANEL2); icon_wrap.config(bg=PANEL2); ic.config(bg=PANEL2); lbl.config(bg=PANEL2)

        for w_ in (row, icon_wrap, ic, lbl):
