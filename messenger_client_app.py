...
в сети (UDP-маячок), регистрирует автозапуск и правило брандмауэра.
Дальше — обычный тихий запуск в фоне.

Pause/Break — открыть/скрыть чат. Если чат скрыт и приходит сообщение —
показывается уведомление (не разворачивает чат поверх игры).
...
# Общий ключ для шифрования переписки на участке клиент-ПК <-> сервер
# администратора (единственный хоп, идущий по реальной сети клуба).
# ВНИМАНИЕ: должен быть 1-в-1 таким же в messenger_admin_app.py.
...
# ── Тёмная тема в стиле Godji — тёплые тона, красный акцент ──────────
BG = '#181113'
HEADER_BG = '#221417'
PANEL_BORDER = '#3a2226'
ACCENT = '#d4172a'
...
_settings_cache = {'notifySound': True, 'showOnlineIndicator': True, 'mutedUntil': 0}
...
# ───────────────────────── скруглённые окна (Windows 11 DWM) ─────────────────────────
...
# ───────────────────────── шифрование текста на сетевом участке ─────────────────────────
def enc_text(plain):
    try:
...
def _send_key(vk, keyup=False):
    ...
    inp = INPUT(type=INPUT_KEYBOARD,
                ki=KEYBDINPUT(wVk=vk, wScan=0, dwFlags=(KEYEVENTF_KEYUP if keyup else 0),
...
def open_windows_emoji_panel(hwnd=None):
    """Открывает встроенную панель эмодзи Windows (Win+.) через SendInput —
    надёжнее устаревшего keybd_event."""
    try:
        if hwnd:
            ctypes.windll.user32.SetForegroundWindow(hwnd)
        _send_key(VK_LWIN)
        _send_key(VK_OEM_PERIOD)
        _send_key(VK_OEM_PERIOD, keyup=True)
        _send_key(VK_LWIN, keyup=True)
    except Exception as e:
        print('[messenger] emoji panel error', e)


def is_muted():
    return time.time() < _settings_cache.get('mutedUntil', 0)
...
def copy_image_to_clipboard(pil_img):
    ...
    try:
        ...
        data = buf.getvalue()[14:]  # без BMP-заголовка — то, что ждёт CF_DIB
        buf.close()
...
# ───────────────────────── Стек уведомлений (максимум 4 видно, остальные — колёсиком) ─────────────────────────
class ToastStack:
    """Единое окно-контейнер в правом нижнем углу. Показывает максимум
    MAX_TOASTS_VISIBLE карточек одновременно; если их больше — над стеком
    появляется прокрутка колёсиком мыши."""
    ...
    def _ensure_window(self):
        ...
        self.canvas = tk.Canvas(win, bg=BG, highlightthickness=0, bd=0,
                                  width=self.WIDTH, height=self.CARD_H * MAX_TOASTS_VISIBLE + self.GAP * MAX_TOASTS_VISIBLE)
        self.canvas.pack()
    ...
    def _reposition(self):
        ...
        h = min(len(self._cards), MAX_TOASTS_VISIBLE) * (self.CARD_H + self.GAP)
        h = max(h, self.CARD_H)
        self.win.geometry('%dx%d+%d+%d' % (self.WIDTH, h, sw - self.WIDTH - 22, sh - h - 60))

    def _rebuild_scrollregion(self):
    ...
    def show(self, title, text, on_click=None):
        ...
        def close(*_):
            ...
            self._rebuild_scrollregion()
            if not self._cards and self.win:
                self.win.destroy()
                self.win = None
        ...
        expiry_id = card.after(TOAST_MS, close)
        self._cards.append((card, expiry_id))
        self._rebuild_scrollregion()
        play_notify_sound()


class Toast:
    """Обёртка для совместимости — используется для системных/приветственных уведомлений."""
    def __init__(self, parent_tk):
...
def build_message_menu(root, text=None, pil_img=None, on_delete=None):
    ...
    if pil_img is not None:
        menu.add_command(label='Скопировать изображение',
                           command=lambda: copy_image_to_clipboard(pil_img))
        def save_as():
    ...
    if on_delete:
        menu.add_separator()
        menu.add_command(label='Удалить у себя', command=on_delete)
    return menu
...
class ChatWindow:
    ...
    def open_emoji_panel(self):
        ...
        self.root.after(80, open_windows_emoji_panel, hwnd)
    ...
    def refresh_title(self):
        try:
            self.title_lbl.config(text='Админ клуба')
    ...
    while True:
        ...
        if base:
            try:
                ...
                _settings_cache['mutedUntil'] = float(s.get('mutedUntil', 0) or 0)
            except Exception:
                pass
...
def nickname_loop(win):
    """Тянет с сервера ник, который ERP-скрипт определил по активной
    сессии на этом ПК (если ручного переименования нет)."""
    while True:
        base = get_base_url()
