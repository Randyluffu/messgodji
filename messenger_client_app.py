...
_cleared_state = {'lastApplied': 0.0}
...
def apply_rounded_corners(win, radius=16):
    try:
        ...
        if _is_windows11_or_later():
            # На Win11 DWM даёт гладкое (со сглаживанием) скругление
            ...
            # На более старых Windows — обрезка по маске (без сглаживания, но работает везде)
            w = win.winfo_width()
            h = win.winfo_height()
...
def detect_pc_name():
    """При автозапуске после перезагрузки сеть может подняться не сразу —
    ждём до ~16 секунд, чтобы не словить временный чужой IP и не завести
    ПК под другим именем/дублем в истории у админа."""
...
class ChatWindow:
    ...
    def toggle_emoji_panel(self):
        ...
        for i, (ch, photo) in enumerate(icons.items()):
            _image_cache.append(photo)
            b = tk.Label(grid, image=photo, bg=HEADER_BG, cursor='hand2', padx=4, pady=4)
    ...
    while True:
        ...
        if base:
            ...
            try:
                ...
                pass
            try:
                rc = _session.get(base + '/clear_state', params={'pc': PC_NAME}, timeout=HTTP_TIMEOUT)
                cleared_at = float(rc.json().get('clearedAt', 0) or 0)
                if cleared_at > _cleared_state['lastApplied']:
                    _cleared_state['lastApplied'] = cleared_at
                    _last_msg_id = 0
                    win.root.after(0, win.clear_local)
            except Exception:
                pass
